...
namespace SynToolkit.Services.ConfigurationServices
{
    ...
    {
        private const string SYNTOOLKIT_STORE_KEY_NAME = @"HKLM\SOFTWARE\SynToolkit\Services\ExtractContextMenu";
        ...
        public bool IsEnabled()
        {
            try
            {
                return _valueNames.All(valueName =>
                    RegistryHelper.GetValue(BLOCKED_KEY_NAME, valueName) is null);
            }
            catch (Exception exception)
            {
                App.logger.Warn($"Unable to detect Extract context-menu state: {exception.Message}");
                return false;
            }
        }
    }
