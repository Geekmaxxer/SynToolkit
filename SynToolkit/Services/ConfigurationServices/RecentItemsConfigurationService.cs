...
namespace SynToolkit.Services.ConfigurationServices
{
    ...
    {
        private const string SYNTOOLKIT_STORE_KEY_NAME = @"HKLM\SOFTWARE\SynToolkit\Services\RecentItems";
        ...
        {
            return !RegistryHelper.IsMatch(EXPLORER_POLICIES_HKCU_KEY_NAME, NO_INSTRUMENTATION_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(EXPLORER_POLICIES_HKCU_KEY_NAME, CLEAR_RECENT_DOCS_ON_EXIT_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(EXPLORER_POLICIES_HKCU_KEY_NAME, NO_RECENT_DOCS_HISTORY_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(EXPLORER_WINDOWS_HKCU_KEY_NAME, NO_REMOTE_DESTINATIONS_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(EXPLORER_POLICIES_HKLM_KEY_NAME, NO_START_MENU_MFU_PROGRAMS_LIST_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(EXPLORER_POLICIES_HKLM_KEY_NAME, NO_RECENT_DOCS_HISTORY_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(EXPLORER_WINDOWS_HKLM_KEY_NAME, SHOW_OR_HIDE_MOST_USED_APPS_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(EXPLORER_WINDOWS_HKLM_KEY_NAME, HIDE_RECENTLY_ADDED_APPS_VALUE_NAME, 1)
                && !RegistryHelper.IsMatch(ADVANCED_EXPLORER_KEY_NAME, START_TRACK_PROGS_VALUE_NAME, 0)
                && !RegistryHelper.IsMatch(ADVANCED_EXPLORER_KEY_NAME, START_TRACK_DOCS_VALUE_NAME, 0);
        }
    }
}
