...
namespace SynToolkit.Services.ConfigurationServices
{
    ...
    {
        private const string SYNTOOLKIT_STORE_KEY_NAME = @"HKLM\SOFTWARE\SynToolkit\Services\UpdateNotifications";
        ...
        {
            return IsAbsentOrZero(WINDOWS_UPADTE_KEY_NAME, SET_AUTO_RESTART_NOTIFICATION_DISABLE_VALUE_NAME)
                && IsAbsentOrZero(WINDOWS_UPADTE_KEY_NAME, SET_UPDATE_NOTIFICATION_LEVEL)
                && (RegistryHelper.IsMatch(UX_SETTINGS_KEY_NAME, RESTART_NOTIFICATIONS_ALLOWED2, null)
                    || RegistryHelper.IsMatch(UX_SETTINGS_KEY_NAME, RESTART_NOTIFICATIONS_ALLOWED2, 1));
        }

        private static bool IsAbsentOrZero(string keyName, string valueName)
        {
            return RegistryHelper.IsMatch(keyName, valueName, null)
                || RegistryHelper.IsMatch(keyName, valueName, 0);
        }
    }
}
